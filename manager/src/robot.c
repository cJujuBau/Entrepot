...
pthread_mutex_t mutexOrdersElements = PTHREAD_MUTEX_INITIALIZER;
...
void* update_robot(void* arg) {
    ...
    while (1) {
        // Get the object to collect
        pthread_mutex_lock(&mutexOrdersElements);// A lock aussi quand on a grandi la liste

        DEBUG_PRINT("Robot %d, nbOrdersElementary: %d\n", rbt->id_robot, ordersElementaryGlobal->nbOrdersElementary);
        if (ordersElementaryGlobal->nbOrdersElementary == 0) {
            printf("No more orders to process\n");
            
            while (deplacementSection(rbt, 0) != 0) {
                sleep(0.5);
            }
            pthread_mutex_unlock(&s_principale[0]->mutex);
            sfCircleShape_setFillColor(rbt->cercle,sfBlack);
            
            pthread_mutex_unlock(&mutexOrdersElements);
            break;
        } else {
            o1 = &(ordersElementaryGlobal->items[0][0]);
            o2 = &(ordersElementaryGlobal->items[0][1]);
            removeOrderFromOrdersGlobal(0);
    
            DEBUG_PRINT("o1: %d, o2: %d\n", o1->id, o2->id);    
        }
        ...
        while (1)
        {
            ...
            else if(rbt->etape == etapeRetourCyclePrincipal2)
            {
                ...
                int section_avant_bac = NOMBRE_SECTIONS_PRINCIPALES - 4 / o1->bac;
